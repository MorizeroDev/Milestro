...
struct Storage<void, E> {
    ...
    template <typename U>
    std::conditional_t<std::is_same_v<U, void>, void, const U&> get() const {
        if constexpr (!std::is_same_v<U, void>)
            return *reinterpret_cast<const U*>(&storage_);
    }

    template <typename U>
    std::conditional_t<std::is_same_v<U, void>, void, U&> get() {
        if constexpr (!std::is_same_v<U, void>)
            return *reinterpret_cast<U*>(&storage_);
    }
...
struct Result {
    ...
    Ret map(Func&& func) const {
        return Details::map(*this, std::forward<Func>(func));
    }
    ...
    Result<T, E> then(Func&& func) const {
        return Details::then(*this, std::forward<Func>(func));
    }

    template <typename Func>
    Result<T, E> otherwise(Func&& func) const {
        return Details::otherwise(*this, std::forward<Func>(func));
    }

    template <typename Func,
              typename Ret = Result<T, typename Details::ResultErrType<typename Details::result_of<Func>::type>::type>>
    Ret orElse(Func&& func) const {
        return Details::orElse(*this, std::forward<Func>(func));
    }
